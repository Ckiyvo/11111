...
def process_file(file_path):
    ...
    if file_extension in ['.pdf']:
        ...
        processor.classify_image(image)
        ...
        processor = AudioProcessor(trans_model_path)
        # 调用 preprocess_audio 和 transcribe_speech 方法进行音频处理和语音识别
        pre_audio = processor.preprocess_audio(file_path)
        processor.transcribe_speech(pre_audio)
    elif file_extension in ['.aac', '.mp4']:
        action_recog_model_path = r"F:\a\apaper\project\project\algorithm\model\cv_TAdaConv_action-recognition"
        processor = VideoProcessor(action_recog_model_path)
        # 调用 preprocess_video 和 action_recognition 方法进行视频预处理和行为识别
        ...
        processor.action_recognition(file_path)
    else:
...
def process_selected_files(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'message': '文件处理成功'})
    return JsonResponse({'message': '无效的请求方法'}, status=400)